...
| Shipping | Quote/selection persistence is covered; real carrier configuration and tracking sync remain. | Freight/tracking can still diverge from fulfillment if no carrier sync exists. | Add/update tracking endpoint or commerce fulfillment sync. |
| Tracking | Buyer hub reads tracking, but operator/update/sync path still needs completion. | Buyer cannot reliably follow delivery after purchase. | Add/update tracking endpoint or commerce fulfillment sync. |
...
1. Add tracking update/sync path.
2. Add support handoff/ticket state.
3. Harden secure embed/runtime validation.
4. Add dashboard pilot metrics and repeatable gates.
