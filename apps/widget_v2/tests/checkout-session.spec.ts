...
test("widget starts and updates the signed checkout session without reading or mutating public carts", async () => {
  ...
  const experience = { items: [{ sku: "real-sku", variant: "with-cheese", name: "Sanduíche", quantity: 2, unit_price: 22.5 }], totals: { subtotal: 45, discount: 5, total: 40 } };
  globalThis.fetch = async (url, init) => {
    ...
    if (request.url.endsWith("/embed/cart")) return Response.json({ session_id: "bound-session", experience: {
      items: [{ ...experience.items[0], quantity: 1 }], totals: { subtotal: 22.5, discount: 0, total: 22.5 },
    } });
