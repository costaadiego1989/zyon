...
function AllocationHistory({ history, loading, error }: { history: PaymentAllocationHistory | null; loading: boolean; error: string | null }) { return <section aria-label="Histórico de alocação do pagamento" style={{ display: "grid", gap: 10, paddingTop: 2 }}><div><span style={kickerStyle}>HISTÓRICO DE ALOCAÇÃO</span><p style={{ margin: "6px 0 0", color: "var(--color-text-faint)", font: "12px/1.5 var(--font-sans)" }}>Plano de divisão e observações do provedor. Não representa saldo disponível ou repasse bancário concluído.</p></div>{loading ? <div role="status" style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--color-text-muted)", font: "12px var(--font-sans)" }}><LoaderCircle size={14} /> Carregando histórico...</div> : null}{error ? <div role="alert" style={{ padding: "10px 12px", borderRadius: 9, background: "var(--color-error-bg)", color: "var(--color-error)", font: "12px/1.5 var(--font-sans)" }}>{error}</div> : null}{history ? <><p style={{ margin: 0, padding: 10, borderRadius: 9, background: "var(--surface-2)", color: "var(--color-text-faint)", font: "11.5px/1.5 var(--font-sans)" }}>{history.scope_note}</p>{history.snapshots.length === 0 ? <p style={{ margin: 0, color: "var(--color-text-faint)", font: "12px var(--font-sans)" }}>Ainda não há observações do provedor para este pagamento.</p> : history.snapshots.map((snapshot) => <AllocationSnapshot key={`${snapshot.kind}-${snapshot.sequence}`} snapshot={snapshot} />)}</> : null}</section>; }
function AllocationSnapshot({ snapshot }: { snapshot: PaymentAllocationHistory["snapshots"][number] }) { const observed = snapshot.kind === "provider_observation"; const state = !observed ? "Planejado" : snapshot.observation_status === "confirmed" ? "Observação confirmada" : "Divergência bloqueada"; const color = !observed ? "var(--color-brand)" : snapshot.observation_status === "confirmed" ? "var(--color-success)" : "var(--color-error)"; return <article style={{ display: "grid", gap: 9, padding: 12, border: "1px solid var(--color-border)", borderRadius: 10, background: "var(--surface-2)" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}><strong style={{ color, font: "700 11px var(--font-mono)", textTransform: "uppercase" }}>{state}</strong><span style={{ color: "var(--color-text-faint)", font: "11px var(--font-mono)" }}>{snapshot.provider} · {formatDateTime(snapshot.confirmed_at ?? snapshot.occurred_at)}</span></div><div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 9 }}><Detail label="Total" value={formatCents(snapshot.planned.gross_cents)} /><Detail label="Taxa da plataforma" value={formatCents(snapshot.planned.platform_fee_cents)} /><Detail label="Repasse planejado" value={formatCents(snapshot.planned.merchant_net_cents)} /><Detail label="Taxa do provedor planejada" value={formatCents(snapshot.planned.provider_fee_cents)} /></div>{snapshot.provider_observed ? <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 9, paddingTop: 9, borderTop: "1px solid var(--color-border)" }}><Detail label="Taxa da plataforma observada" value={snapshot.provider_observed.platform_fee_cents === undefined ? "Não informada" : formatCents(snapshot.provider_observed.platform_fee_cents)} /><Detail label="Líquido observado na conta emissora" value={snapshot.provider_observed.merchant_net_cents === undefined ? "Não informado" : formatCents(snapshot.provider_observed.merchant_net_cents)} /></div> : null}</article>; }
