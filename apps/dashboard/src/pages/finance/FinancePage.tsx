...
export function FinancePage({ apiBaseUrl, me }: { apiBaseUrl: string; me: MerchantProfile | null }) {
  ...
  const [payouts, setPayouts] = useState<FinanceMerchantPayouts | null>(null);
  ...
  const [allocationHistory, setAllocationHistory] = useState<PaymentAllocationHistory | null>(null);
  ...
  const loadPayouts = useCallback(async () => {
    ...
    try {
      setPayouts(await api.getMerchantPayouts());
    } catch (caught) {
      setPayoutsError(displayError(caught, "Não foi possível carregar os repasses."));
    } finally {
  ...
  useEffect(() => {
    const paymentIntentId = selected?.kind === "sale" ? selected.payment_intent_id : null;
    if (!paymentIntentId) {
    ...
    void api.getPaymentAllocationHistory(paymentIntentId)
      .then((history) => { if (active) setAllocationHistory(history); })
      .catch((caught) => { if (active) setAllocationHistoryError(displayError(caught, "Não foi possível carregar o histórico de alocação.")); })
      .finally(() => { if (active) setAllocationHistoryLoading(false); });
    return () => { active = false; };
  }, [api, selected]);
    ...
    <div className="page-container">
      <header className="page-head">
        <div>
          ...
          <h1>Financeiro</h1>
          <p className="page-lead">Acompanhe o valor bruto dos pedidos concluídos e os reembolsos confirmados. Saldo e repasses dependem da conciliação do provedor.</p>
        </div>
...
function Overview({ summary, payouts, payoutsLoading, payoutsError, onTransactions, onRetryPayouts }: { summary: FinanceSummary; payouts: FinanceMerchantPayouts | null; payoutsLoading: boolean; payoutsError: string | null; onTransactions: () => void; onRetryPayouts: () => Promise<void> }) {
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
    ...
    <div className="finance-page__columns">
      <section className="panel" style={{ padding: 0, overflow: "hidden" }}><PanelHeader eyebrow="EVOLUÇÃO" title="Pedidos concluídos e reembolsos" note="Valores brutos em reais" /><RevenueChart points={summary.series} /></section>
      <section className="panel" style={{ padding: 0, overflow: "hidden" }}><PanelHeader eyebrow="PAGAMENTOS" title="Pedidos por meio de pagamento" /><PaymentMethods items={summary.payment_methods} /></section>
    </div>
    <section className="panel" style={{ padding: 0, overflow: "hidden" }}>
      ...
      <div className="finance-page__explainers">
        <Explainer icon={<CheckCircle2 size={16} />} title="Pedidos concluídos" text="Cada pedido aparece uma vez no relatório, com o total bruto registrado na compra." />
        ...
        <Explainer icon={<Landmark size={16} />} title="Conciliação necessária" text="Saldo liquidado, taxas do provedor e repasses confirmados só entram após a conciliação." />
      </div>
    </section>
  </div>;
}

function PayoutTimeline({ payouts, loading, error, onRetry }: { payouts: FinanceMerchantPayouts | null; loading: boolean; error: string | null; onRetry: () => Promise<void> }) {
  return <section className="panel" style={{ padding: 0, overflow: "hidden" }}>
    <PanelHeader eyebrow="REPASSES PROTEGIDOS" title="Agenda de repasses" note="Status do valor líquido do merchant" />
    {loading ? <PageLoader /> : error ? <div style={{ padding: "0 20px 20px" }}><ErrorPanel message={error} onRetry={onRetry} /></div> : payouts?.items.length ? <div style={{ overflowX: "auto" }}>
      <p style={{ margin: "0 20px", padding: "0 0 14px", color: "var(--color-text-faint)", font: "12px/1.5 var(--font-sans)" }}>{payouts.scope_note}</p>
      <table style={{ width: "100%", minWidth: 760, borderCollapse: "collapse" }}><caption className="sr-only">Agenda de repasses do merchant</caption><thead><tr>{["VALOR LÍQUIDO", "ELEGÍVEL EM", "STATUS", "PROVEDOR", "CONFIRMAÇÃO"].map(label => <th key={label} style={tableHeaderStyle}>{label}</th>)}</tr></thead><tbody>{payouts.items.map(payout => <tr key={payout.id}><td style={{ ...tableCellStyle, color: "var(--color-text)", font: "700 13px var(--font-mono)" }}>{formatBrl(payout.amount_brl)}</td><td style={tableCellStyle}>{formatDateTime(payout.eligible_at)}</td><td style={tableCellStyle}><PayoutStatus status={payout.status} /></td><td style={tableCellStyle}>{payout.provider}</td><td style={tableCellStyle}>{payout.confirmed_at ? formatDateTime(payout.confirmed_at) : payout.failure_code ? "Ação necessária" : "Aguardando confirmação"}</td></tr>)}</tbody></table>
    </div> : <div style={{ padding: "0 20px 20px", color: "var(--color-text-faint)", font: "13px/1.55 var(--font-sans)" }}>Não há repasses com retenção nesta conta. Quando essa modalidade estiver ativa, cada pagamento aparecerá aqui.</div>}
  </section>;
}

function PayoutStatus({ status }: { status: string }) {
  const current = status.toLowerCase();
  const labels: Record<string, string> = { held: "Em período de estorno", payout_ready: "Preparando repasse", payout_submitted: "Transferência enviada", released: "Repasse confirmado", payout_failed: "Ação necessária", refunded: "Reembolsado", refund_reversal_required: "Estorno a recuperar", chargebacked: "Contestação", chargeback_debt: "Contestação a recuperar" };
  const color = current === "released" ? "var(--color-success)" : ["payout_failed", "refund_reversal_required", "chargebacked", "chargeback_debt"].includes(current) ? "var(--color-error)" : "var(--color-brand)";
  const background = current === "released" ? "var(--color-success-bg)" : ["payout_failed", "refund_reversal_required", "chargebacked", "chargeback_debt"].includes(current) ? "var(--color-error-bg)" : "var(--color-brand-subtle)";
  return <span style={{ display: "inline-flex", minHeight: 23, alignItems: "center", padding: "0 8px", borderRadius: 999, color, background, font: "700 10px var(--font-mono)" }}>{labels[current] ?? status}</span>;
}
...
function RevenueChart({ points }: { points: FinanceSummary["series"] }) { if (points.length === 0) return <EmptyChart />; const width = 760; const height = 240; const pad = { top: 20, right: 20, bottom: 32, left: 52 }; const max = Math.max(...points.flatMap((point) => [point.completed_orders_gross_brl, point.refunds_brl]), 1); const x = (index: number) => pad.left + (index / Math.max(points.length - 1, 1)) * (width - pad.left - pad.right); const y = (value: number) => height - pad.bottom - (value / max) * (height - pad.top - pad.bottom); const pathFor = (read: (point: FinanceSummary["series"][number]) => number) => points.map((point, index) => `${index === 0 ? "M" : "L"}${x(index)},${y(read(point))}`).join(" "); return <div style={{ padding: "8px 18px 16px" }}><svg viewBox={`0 0 ${width} ${height}`} width="100%" height={240} role="img" aria-label="Evolução de pedidos concluídos e reembolsos no período">{[0, .5, 1].map((factor) => <g key={factor}><line x1={pad.left} x2={width - pad.right} y1={y(max * factor)} y2={y(max * factor)} stroke="var(--color-border)" strokeDasharray="4 6" /><text x={pad.left - 9} y={y(max * factor) + 4} textAnchor="end" fill="var(--color-text-faint)" fontSize="10" fontFamily="var(--font-mono)">{formatCompactBrl(max * factor)}</text></g>)}<path d={pathFor((point) => point.completed_orders_gross_brl)} fill="none" stroke="var(--color-brand)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />{points.some((point) => point.refunds_brl > 0) ? <path d={pathFor((point) => point.refunds_brl)} fill="none" stroke="var(--color-error)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /> : null}{points.map((point, index) => <text key={point.date} x={x(index)} y={height - 10} textAnchor="middle" fill="var(--color-text-faint)" fontSize="10" fontFamily="var(--font-mono)">{shortDate(point.date)}</text>)}</svg><div style={{ display: "flex", gap: 16, padding: "0 4px", color: "var(--color-text-muted)", font: "11px var(--font-sans)" }}><Legend color="var(--color-brand)" label="Pedidos concluídos (bruto)" /><Legend color="var(--color-error)" label="Reembolsos" /></div></div>; }
function PaymentMethods({ items }: { items: FinanceSummary["payment_methods"] }) { if (items.length === 0) return <EmptyChart message="Os métodos aparecem depois dos primeiros pedidos concluídos." />; const total = items.reduce((sum, item) => sum + item.completed_orders_gross_brl, 0) || 1; return <div style={{ padding: "0 20px 20px", display: "grid", gap: 14 }}>{items.map((item) => <div key={item.method}><div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10, marginBottom: 7 }}><span style={{ color: "var(--color-text)", font: "600 13px var(--font-sans)" }}>{item.method}</span><span style={{ color: "var(--color-text-muted)", font: "12px var(--font-mono)" }}>{formatBrl(item.completed_orders_gross_brl)} · {item.orders} pedido{item.orders === 1 ? "" : "s"}</span></div><div style={{ height: 6, borderRadius: 99, background: "var(--surface-3)", overflow: "hidden" }}><div style={{ height: "100%", width: `${Math.max(3, (item.completed_orders_gross_brl / total) * 100)}%`, borderRadius: "inherit", background: "var(--color-brand)" }} /></div></div>)}</div>; }
...
function TransactionDetails({ transaction, allocationHistory, allocationHistoryLoading, allocationHistoryError }: { transaction: FinanceTransaction; allocationHistory: PaymentAllocationHistory | null; allocationHistoryLoading: boolean; allocationHistoryError: string | null }) { const refund = transaction.kind === "refund"; const canShowAllocationHistory = !refund && Boolean(transaction.payment_intent_id); return <div style={{ display: "grid", gap: 18 }}><div style={{ padding: 16, borderRadius: 10, border: "1px solid var(--color-border)", background: "var(--surface-2)" }}><span style={kickerStyle}>{refund ? "REEMBOLSO CONFIRMADO" : "TOTAL BRUTO DO PEDIDO"}</span><strong style={{ display: "block", marginTop: 10, color: refund ? "var(--color-error)" : "var(--color-brand)", font: "700 29px var(--font-mono)" }}>{refund ? "−" : "+"}{formatBrl(Math.abs(transaction.amount_brl))}</strong></div><div style={{ display: "grid", gap: 14 }}><Detail label="Data" value={formatDateTime(transaction.occurred_at)} /><Detail label="Pedido" value={transaction.order_reference} /><Detail label="Método" value={transaction.payment_method ?? "Não informado"} /><Detail label="Situação" value={<StatusBadge kind={transaction.kind} status={transaction.status} />} /><Detail label="Referência do pagamento" value={transaction.payment_intent_id ?? "Não informado"} mono /></div><p style={{ margin: 0, padding: 12, borderRadius: 9, background: "var(--surface-2)", color: "var(--color-text-faint)", font: "12px/1.55 var(--font-sans)" }}>{refund ? "O valor foi incluído após a confirmação do reembolso pelo provedor." : "O valor corresponde ao total bruto registrado no pedido concluído; não é saldo liquidado nem repasse confirmado."}</p>{canShowAllocationHistory ? <AllocationHistory history={allocationHistory} loading={allocationHistoryLoading} error={allocationHistoryError} /> : null}</div>; }
function AllocationHistory({ history, loading, error }: { history: PaymentAllocationHistory | null; loading: boolean; error: string | null }) { return <section aria-label="Histórico de alocação do pagamento" style={{ display: "grid", gap: 10, paddingTop: 2 }}><div><span style={kickerStyle}>HISTÓRICO DE ALOCAÇÃO</span><p style={{ margin: "6px 0 0", color: "var(--color-text-faint)", font: "12px/1.5 var(--font-sans)" }}>Plano de divisão e observações do provedor. O repasse só aparece confirmado com o recibo de transferência do provedor.</p></div>{loading ? <div role="status" style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--color-text-muted)", font: "12px var(--font-sans)" }}><LoaderCircle size={14} /> Carregando histórico...</div> : null}{error ? <div role="alert" style={{ padding: "10px 12px", borderRadius: 9, background: "var(--color-error-bg)", color: "var(--color-error)", font: "12px/1.5 var(--font-sans)" }}>{error}</div> : null}{history ? <><p style={{ margin: 0, padding: 10, borderRadius: 9, background: "var(--surface-2)", color: "var(--color-text-faint)", font: "11.5px/1.5 var(--font-sans)" }}>{history.scope_note}</p>{history.snapshots.length === 0 ? <p style={{ margin: 0, color: "var(--color-text-faint)", font: "12px var(--font-sans)" }}>Ainda não há observações do provedor para este pagamento.</p> : history.snapshots.map((snapshot) => <AllocationSnapshot key={`${snapshot.kind}-${snapshot.sequence}`} snapshot={snapshot} />)}</> : null}</section>; }
function AllocationSnapshot({ snapshot }: { snapshot: PaymentAllocationHistory["snapshots"][number] }) { const observed = snapshot.kind === "provider_observation"; const merchantTransfer = snapshot.allocations.find((entry) => entry.key === "merchant_payout" && entry.provider_transfer_id); const transferConfirmed = Boolean(merchantTransfer && snapshot.observation_status === "confirmed"); const state = !observed ? "Planejado" : transferConfirmed ? "Repasse confirmado" : snapshot.observation_status === "confirmed" ? "Observação confirmada" : "Divergência bloqueada"; const color = !observed ? "var(--color-brand)" : snapshot.observation_status === "confirmed" ? "var(--color-success)" : "var(--color-error)"; return <article style={{ display: "grid", gap: 9, padding: 12, border: "1px solid var(--color-border)", borderRadius: 10, background: "var(--surface-2)" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}><strong style={{ color, font: "700 11px var(--font-mono)", textTransform: "uppercase" }}>{state}</strong><span style={{ color: "var(--color-text-faint)", font: "11px var(--font-mono)" }}>{snapshot.provider} · {formatDateTime(snapshot.confirmed_at ?? snapshot.occurred_at)}</span></div><div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 9 }}><Detail label="Total" value={formatCents(snapshot.planned.gross_cents)} /><Detail label="Taxa da plataforma" value={formatCents(snapshot.planned.platform_fee_cents)} /><Detail label="Repasse planejado" value={formatCents(snapshot.planned.merchant_net_cents)} /><Detail label="Taxa do provedor planejada" value={formatCents(snapshot.planned.provider_fee_cents)} /></div>{snapshot.provider_observed ? <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 9, paddingTop: 9, borderTop: "1px solid var(--color-border)" }}><Detail label="Taxa da plataforma observada" value={snapshot.provider_observed.platform_fee_cents === undefined ? "Não informada" : formatCents(snapshot.provider_observed.platform_fee_cents)} /><Detail label={transferConfirmed ? "Líquido repassado ao merchant" : "Líquido observado na conta emissora"} value={snapshot.provider_observed.merchant_net_cents === undefined ? "Não informado" : formatCents(snapshot.provider_observed.merchant_net_cents)} />{transferConfirmed ? <Detail label="Recibo da transferência" value={merchantTransfer?.provider_transfer_id ?? "Não informado"} mono /> : null}</div> : null}</article>; }
...
function formatCents(value: number): string { return formatBrl(value / 100); }
