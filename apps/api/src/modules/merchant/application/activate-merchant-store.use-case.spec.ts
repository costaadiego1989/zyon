...
test("activating a permitted store issues a session scoped to that store", async () => {
  ...
  const useCase = new ActivateMerchantStoreUseCase(
    {
      activate: async (...input: unknown[]) => {
        ...
        return { merchantId: "store_2", role: "admin" };
      },
    } as never,
    {
      issue: async (...input: unknown[]) => {
        issueCalls.push(input);
        return "session-for-store-2";
      },
      expiresIn: () => 3600,
    } as never,
  );
  ...
  assert.deepEqual(activationCalls, [[{
    userId: "user_1", merchantId: "store_1", role: "owner",
  }, "store_2"]]);
  assert.deepEqual(issueCalls, [[{
    userId: "user_1", merchantId: "store_2", email: "owner@example.test", role: "admin", authVersion: 3,
  }, 3]]);
});
