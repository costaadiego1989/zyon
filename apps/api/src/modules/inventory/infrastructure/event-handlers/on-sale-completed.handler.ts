  ...
  async handle(event: SaleCompletedEvent): Promise<void> {
    ...
    for (const item of items) {
      try {
        ...
        if (this.catalogStock) {
          try {
            const res = await this.catalogStock.decrementBySku(merchantId, item.sku, item.quantity);
            if (!res.ok) {
              this.logger.warn(`catalog ProductStock not decremented (not found/insufficient)`, { merchantId, sku: item.sku, quantity: item.quantity });
            }
          } catch (e) {
            this.logger.warn(`catalog ProductStock decrement failed: ${e instanceof Error ? e.message : String(e)}`, { merchantId, sku: item.sku });
          }
        }
