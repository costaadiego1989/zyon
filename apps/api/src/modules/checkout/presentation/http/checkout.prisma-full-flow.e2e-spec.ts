...
function buildController(repo: InMemoryCheckoutRepository) {
  ...
  const ctrl = new CheckoutController(
    createStartCheckoutUseCase(repo, repo, { merchantRepository: repo }), new TrackCheckoutEventUseCase(repo, repo),
...
test("E2E Prisma Full Flow: data_collection → shipping → payment → completed", async () => {
  ...
  const intent = await new CreatePaymentIntentUseCase(repo, repo, payments, new FakePaymentProvider()).execute({
