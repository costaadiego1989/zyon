...
describe("GetPaymentAllocationHistoryUseCase", () => {
  it("returns only the authenticated merchant's immutable plan and provider observations", async () => {
    ...
    assert.match(history.scope_note, /repasse ao merchant s. est. confirmado quando a aloca..o dele traz o identificador da transfer.ncia/i);
    assert.match(history.scope_note, /n.o representa saldo dispon.vel para saque banc.rio ou concilia..o completa/i);
