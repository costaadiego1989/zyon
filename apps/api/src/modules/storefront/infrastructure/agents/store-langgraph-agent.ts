...
export class StorefrontLangGraphAgent {
  ...
  private buildDefaultSystem(merchantName?: string, storeCategory?: string, storeSettings?: Record<string, any>, agentIdentity?: { agentName?: string; persona?: string; tone?: string; greeting?: string }, merchantPolicy?: { maxDiscountPercent?: number; allowFreeShipping?: boolean; allowShippingDiscount?: boolean; freeShippingMinCartValue?: number; maxPartialShippingDiscount?: number; offerExpirationMinutes?: number }, advancedRules?: string[]): string {
    ...
    return [
      ...
      "- Quando o cliente diz 'Adicionar X ao carrinho': use search_products com o nome exato do produto para obter o ID, depois use add_item_to_cart com variantId=ID_DO_PRODUTO (campo 'id' do resultado) e quantity=1.",
      "- O campo 'variantId' do add_item_to_cart aceita o campo 'id' do produto retornado por search_products. NÃO precisa procurar dentro de variants — passe o product.id diretamente.",
      "- Se o produto já apareceu no histórico (search_products ou get_product_details foi chamado antes), use o 'id' daquele resultado direto — NÃO busque novamente.",
      "- NÃO peça confirmação de variante, cor ou tamanho — use o primeiro resultado disponível.",
      "- Após add_item_to_cart retornar sucesso: responda EXATAMENTE '{nome_do_produto} adicionado ao carrinho!' — nada mais.",
      "- Se add_item_to_cart retornar error 'out_of_stock': responda 'Produto indisponível no momento.'",
      "",
