-- Immutable settlement ledger. This deploy-path migration deliberately uses
-- idempotent DDL because earlier production deployments carried the code while
-- their SQL lived in the archived migration directory.
CREATE UNIQUE INDEX IF NOT EXISTS "payment_intents_id_merchant_id_key"
...
CREATE TABLE IF NOT EXISTS "payment_settlements" (
  ...
  CONSTRAINT "payment_settlements_amounts_nonnegative_check" CHECK (
    "planned_gross_cents" >= 0 AND "planned_platform_fee_cents" >= 0
    AND "planned_merchant_net_cents" >= 0 AND "planned_provider_fee_cents" >= 0
...
CREATE UNIQUE INDEX IF NOT EXISTS "payment_settlements_id_merchant_id_key"
  ON "payment_settlements"("id", "merchant_id");
CREATE UNIQUE INDEX IF NOT EXISTS "payment_settlements_merchant_id_payment_intent_id_sequence_key"
  ON "payment_settlements"("merchant_id", "payment_intent_id", "sequence");
CREATE UNIQUE INDEX IF NOT EXISTS "payment_settlements_merchant_id_provider_provider_settlement_id_key"
  ON "payment_settlements"("merchant_id", "provider", "provider_settlement_id");
CREATE INDEX IF NOT EXISTS "payment_settlements_merchant_id_payment_intent_id_occurred_at_idx"
  ON "payment_settlements"("merchant_id", "payment_intent_id", "occurred_at");
CREATE INDEX IF NOT EXISTS "payment_settlements_merchant_id_status_occurred_at_idx"
  ON "payment_settlements"("merchant_id", "status", "occurred_at");
CREATE INDEX IF NOT EXISTS "payment_settlements_provider_status_occurred_at_idx"
...
CREATE UNIQUE INDEX IF NOT EXISTS "payment_settlement_entries_merchant_id_settlement_id_sequence_key"
  ON "payment_settlement_entries"("merchant_id", "settlement_id", "sequence");
CREATE UNIQUE INDEX IF NOT EXISTS "payment_settlement_entries_merchant_id_settlement_id_entry_key_key"
  ON "payment_settlement_entries"("merchant_id", "settlement_id", "entry_key");
CREATE UNIQUE INDEX IF NOT EXISTS "payment_settlement_entries_merchant_id_provider_provider_transfer_id_key"
  ON "payment_settlement_entries"("merchant_id", "provider", "provider_transfer_id");
CREATE INDEX IF NOT EXISTS "payment_settlement_entries_merchant_id_status_occurred_at_idx"
  ON "payment_settlement_entries"("merchant_id", "status", "occurred_at");
CREATE INDEX IF NOT EXISTS "payment_settlement_entries_merchant_id_entry_type_occurred_at_idx"
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'payment_settlements_append_only') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'payment_settlement_entries_append_only') THEN
