...
describe("CartJourneyStepper", () => {
  it("renders horizontal cart journey rail with track fill", () => {
    ...
    expect(document.querySelector(".aacp-cart-journey--horizontal")).not.toBeNull();
    expect(document.querySelector(".aacp-cart-journey-track-fill")).not.toBeNull();
    expect(screen.getByText("Carrinho")).toBeTruthy();
    expect(screen.getByText("Dados")).toBeTruthy();
    expect(document.querySelector(".aacp-cart-journey-step.active .aacp-cart-journey-label")?.textContent).toBe("Dados");
  });

  it("starts at carrinho when there are no items", () => {
    render(<CartJourneyStepper checkoutStage="data_collection" itemCount={0} />);
    expect(document.querySelector(".aacp-cart-journey-step.active .aacp-cart-journey-label")?.textContent).toBe("Carrinho");
  });
});
