...
export function stageNarrative(stage: CheckoutExperienceSnapshot["stage"], nextField?: string): string {
  switch (stage) {
    case "data_collection":
      if (nextField === "nome") return "Para comecar, vou confirmar seus dados pessoais e manter o pedido sincronizado.";
      if (nextField === "email") return "Vou validar seu email para enviar codigo, nota e confirmacao do pedido.";
      if (nextField === "CPF") return "Dados fiscais entram somente quando a loja precisa emitir a nota com seguranca.";
      if (nextField === "telefone") return "O telefone ajuda no rastreio e na comunicacao de entrega.";
      return "Vamos fechar seu cadastro antes de negociar preco, frete ou cupom.";
    case "shipping":
      if (nextField === "CEP") return "Agora eu valido o CEP e busco as opcoes reais de entrega.";
      if (nextField === "confirmar CEP") return "Confirme o endereco para eu cotar frete sem erro.";
      if (nextField?.includes("numero")) return "Falta o numero/complemento para concluir a cotacao.";
      return "Endereco em validacao. Em seguida mostro frete e prazo disponiveis.";
    case "payment":
      return "Escolha PIX ou cartao. O link seguro e gerado somente depois da confirmacao.";
    case "completed":
      return "Pedido confirmado. Detalhes, rastreio e resumo ficam disponiveis no fechamento.";
    default:
...
export function agentTypingLine(agentFullName: string): string {
  const trimmed = agentFullName.trim();
  if (!trimmed) return "Assistente esta digitando";
  return `${trimmed} esta digitando`;
}
