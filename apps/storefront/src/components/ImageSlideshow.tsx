...
export default function ImageSlideshow({ images, alt = "", width = "100%", height = "100%", objectFit = "contain", borderRadius = "0", showDots = true, showArrows = true, autoPlay = false, autoPlayInterval = 5000 }: SlideShowProps) {
  ...
  return <div className={styles.gallery} style={{ width, height, borderRadius, touchAction: count > 1 ? "pan-y pinch-zoom" : "auto" }} {...swipe} role="group" aria-label={"Imagens de " + alt}>
    ...
    {count > 1 && showArrows ? <>
      <button data-neu="control" type="button" className={styles.arrow + " " + styles.previous} aria-label="Imagem anterior" onClick={(event) => { event.stopPropagation(); move(-1); }}><FiChevronLeft aria-hidden="true" /></button>
      <button data-neu="control" type="button" className={styles.arrow + " " + styles.next} aria-label="Próxima imagem" onClick={(event) => { event.stopPropagation(); move(1); }}><FiChevronRight aria-hidden="true" /></button>
    </> : null}
    {count > 1 && showDots ? <div className={styles.dots} aria-label="Escolher imagem">{images.map((_, i) => <button data-neu="text" key={i} type="button" aria-label={"Imagem " + (i + 1)} aria-pressed={i === index} onClick={(event) => { event.stopPropagation(); setCurrent(i); }}><span /></button>)}</div> : null}
  </div>;
}
