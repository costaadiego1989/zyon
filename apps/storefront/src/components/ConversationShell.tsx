        ...
        <>
          <div ref={threadRef} style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "20px 18px", display: "flex", flexDirection: "column", gap: "14px", minHeight: 0, scrollBehavior: "smooth" }}>
            ...
            {messages.map((m) => {
              if (m.role === "agent") {
                return (
                  <div key={m.id} style={{ display: "flex", gap: "9px", alignItems: "flex-end", maxWidth: "min(82%, 520px)", alignSelf: "flex-start", animation: "bubble-in 0.28s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
                    <div style={{ width: "26px", height: "26px", borderRadius: "50%", background: `radial-gradient(120% 120% at 30% 25%, rgba(255, 255, 255, 0.92), rgba(255, 255, 255, 0) 42%), var(--aacp-accent)`, flex: "none", position: "relative" }}>
