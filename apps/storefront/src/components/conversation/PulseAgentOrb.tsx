export function PulseAgentOrb({ size = 96, avatarUrl }: { size?: number; avatarUrl?: string }) {
  ...
  return (
    <div aria-hidden style={{ position: "relative", width: size, height: size, flexShrink: 0, animation: "orbFloat 6s ease-in-out infinite" }}>
      ...
      {avatarUrl ? (
        // Merchant-branded avatar replaces the CSS orb sphere+eyes, keeping the
        // same ring, glow, float and size so the agent identity stays on-brand.
        <img
          src={avatarUrl}
          alt=""
